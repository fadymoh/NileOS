...
{
    if (service == xmlService_t)
    {
        printk("Trying to dispatch:\n");
        printk("kernel_services: %x\n", services_of_kernel->kernel_services[service]);
        printk("kernel_services method: %x\n", services_of_kernel->kernel_services[service]->service_method[method_num]);
    }
