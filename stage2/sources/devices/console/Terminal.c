...
void terminal_initialize(Terminal *p_terminal, uint16_t *p_terminal_buffer, uint8_t p_x_max, uint8_t p_y_max, uint16_t p_y_rows)
{
    ...
    if (p_terminal_buffer != NULL)
        p_terminal->terminal_buffer = p_terminal_buffer;
    // else allocate
...
void terminal_putCharacterAndAdvanceString(Terminal *p_terminal, const char p_character, uint8_t p_background_color, uint8_t p_foreground_color)
{
    if (p_character == '\n')
    {
...
void terminal_putString(Terminal *p_terminal, const char *p_string, uint8_t p_background_color, uint8_t p_foreground_color)
{
    uint64_t i;
...
void terminal_putHexa(Terminal *p_terminal, uint64_t p_hexa, uint8_t p_background_color, uint8_t p_foreground_color, bool p_show_x_flag)
{
    uint64_t l_tmp;
...
void terminal_putHexa128(Terminal *p_terminal, uint128_t p_hexa, uint8_t p_background_color, uint8_t p_foreground_color, bool p_show_x_flag)
{
    uint8_t l_tmp;
    bool l_no_zeros = true;
    int i;
...
{
    if (p_terminal->y_position < p_terminal->y_max)
        return p_terminal->terminal_buffer;
    else
        return (p_terminal->terminal_buffer + ((p_terminal->y_position - p_terminal->y_max) * p_terminal->x_max));
}