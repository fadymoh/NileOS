...
char * strstr (char * p_haystack,const char * p_needle)
{
    ...
    while (temp+needle_len <= haystack_end)
    {
        ...
        else if ( ptr == temp) temp++;        
        else if ( ptr != NULL)
        {
            char * ptr1 = rscanc(p_needle,ptr[0],ptr-temp);
            if ( ptr1 == NULL) temp += needle_len;
            else temp += strlen(ptr1)-1;
        };
    }
    return NULL;
}
...
void * scanmem (void * p_haystack,const void * p_needle,uint64_t p_haystack_len,uint64_t p_needle_len)
{
    ...
    char * temp= p_haystack;
    if ( p_haystack_len < p_needle_len) return NULL;
    while (temp+p_needle_len <= haystack_end)
    {
        ...
        else /*if ( ptr != NULL)*/
        {
            ...
            if ( ptr1 == NULL) temp ++;//temp += p_needle_len;
            else temp += ((p_needle_len -(ptr1-p_needle))-1);
        } 
    }
    return NULL;
...
void * strcmp (char * p_dest,const char * p_src)
{
    ...
    uint64_t len = l_dest;
    if ( l_src < l_dest) len = l_src;
    void * result  = memcmp(p_dest,p_src,len);
    if ( result !=NULL ) return result;
    if ( l_src < l_dest) return p_dest+(len-1);
    else if ( l_src > l_dest) return ((char *)p_src)+(len-1);
    else return NULL;
}
...
int lstrcmp (char * p_dest,const char * p_src)
{
    uint64_t l_dest = strlen(p_dest);
    uint64_t l_src = strlen(p_src);
    if (l_dest > l_src) return 1;
    else if (l_dest < l_src) return -1;
    uint64_t len = l_dest;
    char * diff = memcmp (p_dest,p_src,len);
    if ( diff == NULL ) return 0;
    else if (p_dest[diff-p_dest] > p_src[diff-p_dest]) return 1;
    else return -1;
